 ...
 * using a command like: java aiproj.slider.Referee 6 your.package.PlayerName
 * your.package.PlayerName to play your program PlayerName against itself on a
 * board of size N=6 See the specification for more detialed instructions
 */
public class Referee {
	...
	public static void main(String[] args) {
		...
		long[] times = new long[] { 0, 0 }; // cumulative time spent by each
											// player
		...
		if (board.finished()) {
			...
			System.out.println("illegal move: " + (turn == Player.H ? "horizontal" : "vertical"));
	...
	/**
	 * Referee's (simplified) internal representation of the board, handles
	...
	private static class Board {
				...
				for (int i = 0; i < n; i++) {
					for (int j = 0; j < n; j++) {
						if (grid[i][j] == turn && canMove(i, j)) {
							throw new IllegalMoveException("can't pass, moves remain!");
						}
					}
				}
		...
		private boolean canMove(int i, int j) {
			if (grid[i][j] == Piece.HSLIDER) {
				...
				return (i + 1 == n) || (grid[i + 1][j] == Piece.BLANK) || (j + 1 < n && grid[i][j + 1] == Piece.BLANK)
						|| (j - 1 >= 0 && grid[i][j - 1] == Piece.BLANK);

			} else if (grid[i][j] == Piece.VSLIDER) {
				// for VSLIDERs, check up, right, and left
				return (j + 1 == n) || (grid[i][j + 1] == Piece.BLANK) || (i + 1 < n && grid[i + 1][j] == Piece.BLANK)
